let achievm=[];
let achievinf=[];
let gameId="553850";//jakoś zimportować od Tomka po wybraniu gry 
let gameNames="Dark souls";//same here
async function getAchievments(){//just achievments, not for specific user
    const response= await fetch("./b.json")//https://api.steampowered.com/ISteamUserStats/GetSchemaForGame/v2/?key=4E05509263A433352E2117FB636D74AA&appid=${gameId}
    return await response.json();
}
async function getAchievmentInfo(){//whether is unlocked or not
    const response= await fetch("./a.json")//https://api.steampowered.com/ISteamUserStats/GetPlayerAchievements/v0001/?appid=${gameId}&key=4E05509263A433352E2117FB636D74AA&steamid=76561198197267374&format=json&include_appinfo=1
    return await response.json();
}
...
gameIMG.id = "gameIMG";
gameIMG.src = `https://shared.fastly.steamstatic.com/store_item_assets/steam/apps/${gameId}/library_600x900.jpg`;// Tu zmiana żeby nie było stałę
gameIMG.alt = "No Image here";
link.appendChild(gameIMG);
const bg = document.createElement("img");
bg.id = "gamebgrd";
bg.src = `https://shared.fastly.steamstatic.com/store_item_assets/steam/apps/${gameId}/library_hero.jpg`;// Tu zmiana żeby nie było stałę, chyba musze tez fetchowac to co tomek 
